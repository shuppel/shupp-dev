...
import { restoreSelection, matchesProject, composeBrief } from '../public/acrylics/site-state.mjs';
...
test('stored collections reject corrupt data and retired project IDs', () => {
  ...
  assert.deepEqual([...restoreSelection('["red","deleted",null,"green","red"]', valid)], ['red', 'green']);
});
...
test('brief includes selected references, exact visitor text, and canonical URLs', () => {
  const draft = composeBrief({ name: ' Alex ', email: 'alex@example.com', interest: 'A website', idea: 'A & B + a little <texture>' }, [{ title: 'Void', href: '/design/void' }]);
