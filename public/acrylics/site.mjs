...
import { restoreSelection, matchesProject, composeBrief } from './site-state.mjs';
...
try { selected = restoreSelection(localStorage.getItem(storageKey), validIds); }
...
function selectedProjects() { return projects.filter(project => selected.has(project.id)); }
...
function updateCollection() {
  ...
  const pigment = saved.length ? mix(...saved.map(project => [new Color(project.pigment), 1 / saved.length])).toString() : '#e2b72d';
  ...
  document.querySelectorAll('[data-save]').forEach(button => {
    ...
    button.setAttribute('aria-label', `${active ? 'Unsave' : 'Save'} ${project.title}`);
    ...
    if (!label) button.textContent = active ? 'Saved to collection ✓' : 'Save to collection +';
  });
  document.querySelector('#collection-summary').textContent = saved.length ? `${saved.length} ${saved.length === 1 ? 'piece' : 'pieces'}. Your mix.` : 'A fresh start.';
  document.querySelector('#collection-mixture').textContent = saved.length ? `${[...new Set(saved.map(project => project.category))].join(' + ')} · one shared palette.` : 'Save a project to begin your collection.';
  const list = document.querySelector('#saved-projects');
  list.replaceChildren(...saved.map(project => {
    ...
    remove.setAttribute('aria-label', `Remove ${project.title} from collection`);
    item.append(link, remove);
    return item;
  }));
  document.querySelector('#clear-collection').disabled = !saved.length;
  document.querySelector('#storage-note').textContent = storageAvailable ? 'Saved in this browser. Yours to clear.' : 'Browser storage is unavailable. This collection lasts for this visit.';
}
function toggleSave(id) {
  ...
  persistSelection();
  updateCollection();
  notify(`${project.title} ${removing ? 'removed from' : 'added to'} your collection.`);
}
function applyFilter() {
  ...
  document.querySelector('#result-status').textContent = `${count} ${count === 1 ? 'project' : 'projects'} / ${filter === 'all' ? 'all work' : filter.toLowerCase()}${query.trim() ? ` matching “${query.trim()}”` : ''}`;
...
function showProject(id) {
  ...
  nodes.push(element('p', project.category, 'detail-meta'), element('p', project.description));
  const actions = element('div', undefined, 'dialog-actions');
  const link = element('a', 'Explore the project ↗', 'ac-button ac-button--ink');
  link.href = project.href;
  const save = element('button', 'Save to collection +', 'ac-button ac-button--cream');
  save.type = 'button';
...
document.addEventListener('click', event => {
  ...
  if (button.dataset.remove) {
    ...
    (nextButtons[Math.min(index, nextButtons.length - 1)] ?? document.querySelector('#collection-to-brief')).focus();
  }
  if (button.dataset.filter) { filter = button.dataset.filter; applyFilter(); }
  if (button.dataset.open) openDialog(button.dataset.open);
  if (button.dataset.details) showProject(button.dataset.details);
});
document.querySelector('#project-search').addEventListener('input', event => { query = event.target.value; applyFilter(); });
...
document.querySelector('#clear-collection').addEventListener('click', () => {
  selected.clear(); persistSelection(); updateCollection();
...
document.querySelector('#copy-brief').addEventListener('click', async () => {
  ...
  try {
    ...
    document.querySelector('#copy-status').textContent = 'Draft copied. Ready when you are.';
  } catch {
    preview.focus(); preview.select();
    document.querySelector('#copy-status').textContent = 'Select and copy the draft above. Clipboard access is unavailable.';
  }
});
...
updateCollection();
document.querySelectorAll('.work-toolbar, #result-status, #brief-form, [data-save], [data-details], [data-open]').forEach(node => { node.hidden = false; });
