...
export function restoreSelection(raw, validIds) {
  try {
    ...
    return new Set(Array.isArray(value) ? value.filter(id => typeof id === 'string' && validIds.has(id)) : []);
  } catch {
    return new Set();
  }
}
...
export function composeBrief({ name, email, interest, idea }, projects) {
  const references = projects.length ? projects.map(project => `- ${project.title}: ${new URL(project.href, 'https://shupp.dev').href}`).join('\n') : 'No project references selected.';
